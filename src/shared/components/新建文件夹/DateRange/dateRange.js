...
function pickerDateRange(inputId, options) {
    ...
    var wrapper = {
        ...
        ta: [
            '<div id="' + this.calendarId + '" class="ta_calendar ta_calendar2 cf" style="background-color:#fff;">',
    ...
    $('#' + this.submitBtn).bind('click', function () {
        ...
        if (!$('#' + that.endDateId).val()) {
            $('#' + that.endDateId).val($('#' + that.startDateId).val());
        }
    ...
    // 为输入框添加click事件
    $('#' + this.inputId).off('click.inputId').on('click.inputId', function () {
        __method.init();
...
pickerDateRange.prototype.show = function (isCompare, __method) {
    ...
    if(this.mOpts.position == "right") { // 固定在input的右边  --  top 为 input.top   left为input.left + input.width
        left = left + $("#" + this.inputId).width()+ 10;
    } else {
        top = pos.top + (__method.mOpts.theme == 'ta' ? 35 : 22) + 'px';
    }

    $("#" + this.calendarId).css('left', left + 'px');
    $("#" + this.calendarId).css('top', top);
